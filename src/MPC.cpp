...
class FG_eval {
 ...
 // TODO: implement MPC
    ...
    double delta_w = 100;
    double a_w = 10;
    double delta_change_w = 100;
    double a_change_w = 100;
    double break_w = 2000;
    
     // The part of the cost based on the reference state.
    for( int i = 0; i < N; i++ ) {
      fg[0] += cte_w*CppAD::pow(vars[cte_start + i] - ref_cte, 2);
      fg[0] += epsi_w*CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
    ...
    for (int i = 0; i< N - 1; i++) {
      ...
      fg[0] += break_w*CppAD::pow((vars[v_start + i] * vars[delta_start + i]), 2);
    }
